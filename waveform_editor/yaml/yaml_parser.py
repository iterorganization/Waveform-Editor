...
TOP_LEVEL_KEYS = {"version", "dd_version", "imports", "waveforms"}
    ...
    def load_yaml(self, yaml_str):
        ...
        if unknown:
            raise ValueError(
                ...
                "Waveforms belong in groups under 'waveforms:'."
            )

        header = {
            key: yaml_data[key]
            for key in ("version", "dd_version", "imports")
            if yaml_data.get(key) is not None
        }
        if "version" in header:
            header["version"] = str(header["version"])
        if "imports" in header:
            header["imports"] = dict(header["imports"])
        self.config.globals.set_globals(header)

        for group_name, group_content in (yaml_data.get("waveforms") or {}).items():
