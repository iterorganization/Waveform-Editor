...
* Whether NICE extracts a linearized model. When *Extract linearized model* is enabled, NICE writes the A, B and C matrices of
  the linearized model as text files, relative to the directory the waveform editor was
  started from. In direct mode these are ``output/linModA.txt``, ``output/linModB.txt``
  and ``output/linModC.txt``, which are overwritten on every run. In inverse mode they
  are placed in a per-time subdirectory: ``output/linearModel_t<time>/linModA.txt``,
  and so on.
